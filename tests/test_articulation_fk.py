...
class Robot:
    ...
    def __init__(self, adapter="cpu"):
        ...
        if True:
            ...
            X_pj = df.transform(
                (w * 2.0, 0.0, 0.0), df.quat_from_axis_angle((0.0, 0.0, 1.0), 0.0)
            )
    ...
    def loss(self, render=True):
        ...
        if True:
            self.state.body_X_sc, self.state.body_X_sm = df.adjoint.launch(
                df.eval_rigid_fk,
                1,
                [  # inputs
                ...
                [self.state.body_X_sc, self.state.body_X_sm],  # outputs
                adapter="cpu",
            ...
            if mode == "gd":
                ...
                if mode == "sgd":
                    optimizer = torch.optim.SGD(
                        params, lr=self.train_rate, momentum=0.8
                    )
