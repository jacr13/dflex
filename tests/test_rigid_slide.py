...
class RigidSlide:
    ...
    def __init__(self, adapter="cpu"):
        ...
        self.vel = torch.tensor(
            (1.0, 0.0, 0.0), dtype=torch.float32, device=adapter, requires_grad=True
        )
    ...
    def loss(self, render=True):
        ...
        self.model.joint_qd = torch.cat(
            (
                torch.tensor(
                    (0.0, 0.0, 0.0), dtype=torch.float32, device=self.model.adapter
                ),
                self.vel,
            )
        )
